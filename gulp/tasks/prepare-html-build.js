module.exports = () => {
  $.gulp.task("prepareHtmlBuild", function() {
    ...
    for (let k = 0; k < tmpFiles.length; k++) {
      ...
      const hbs = $.fs
        .readFileSync($.config.tmpPath + "/html/" + tpmTemplateName + ".html")
        .toString();

      if ($.argv._[0] === "build") {
        $.fs.writeFileSync(
          $.config.tmpPath + "/html/" + tpmTemplateName + ".html",
          hbs.replace(
            /<title>(.*)/,
    ...
    for (let j = 0; j < files.length; j++) {
      ...
      html +=
        ...
        pageName +
        "</div></div>" +
        '<a class="page-default__item js-hover-item" title="' +
        pageName +
        '" href="' +
        desc +
